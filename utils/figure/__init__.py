...
class Figure():
	...
	"""
	def __init__(self, ax : plt.Axes, dataset : nc.Dataset, map_options : dict):
		self.ax = ax
		...
		try:
			self.lons = dataset['lon'][:]
			self.lats = dataset['lat'][:]
		except:
			self.lons = dataset['longitude'][:]
			self.lats = dataset['latitude'][:]
	...
	def plot_data(self):
		...
		if self.map_options['colorbar']:
			plt.colorbar(mappable=mesh, ax=self.ax, orientation='horizontal')
